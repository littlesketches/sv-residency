...
chart.methods.misc.applyQuerySettings = async (settings) => {
    ...
    settings.axis.x.start = settings.queryParameters.get('start') ? settings.queryParameters.get('start') :  dateArray[dateArray.length - settings.axis.x.defaultRange]
    settings.axis.x.end = settings.queryParameters.get('end') ? settings.queryParameters.get('end') :  dateArray[dateArray.length-1]
};
